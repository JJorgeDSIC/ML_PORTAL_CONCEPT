...
var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
    url: "/problems/upload_test/upload/", // Set the url
    thumbnailWidth: 80,
    thumbnailHeight: 80,
    parallelUploads: 1,
      ...
      if(response.success == true){
          ...
          error: function(file, response) {
            ...
            if($.type(response) === "string"){
              var message = "Unexpected Error"; //dropzone sends it's own error messages in string
            }else{
              file.previewElement.classList.add("dz-error");
...
function requestUploadTrainingStatus() {
  ...
  $.ajax({
    ...
    url : '/problems/upload_test/check/',
...
function deleteFile() {
  ...
  $.ajax({
    // la URL para la petición
    url : '/problems/upload_test/delete/',
